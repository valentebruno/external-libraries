...
CXX=aarch64-linux-gnu-gcc CFLAGS="-fPIC -O3" ./configure --prefix="${EXTERNAL_LIBRARY_DIR}/zlib-${ZLIB_VERSION}" --static
make && make install
cd ..
...
CC=aarch64-linux-gnu-gcc LD=aarch64-linux-gnu-ld AS=aarch64-linux-gnu-as AR=aarch64-linux-gnu-ar ./Configure -fPIC --prefix="${EXTERNAL_LIBRARY_DIR}/openssl" ${OPENSSL_OS} no-asm enable-static-engine
make -k || true
make -k install || true
# docs fail, but hopefully everything else important makes it through
