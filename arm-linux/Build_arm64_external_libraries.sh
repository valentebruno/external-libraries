...
rm -fr libusb
git clone --branch leap-2.2.x git@github.com:leapmotion/libusb.git
cd libusb
./bootstrap.sh
...
mkdir -p Build
cd Build
cmake .. -DCMAKE_TOOLCHAIN_FILE=~/platform/toolchain-arm64.cmake -DCMAKE_BUILD_TYPE:STRING=Release -DCMAKE_INSTALL_PREFIX:PATH="${EXTERNAL_LIBRARY_DIR}/libuvc" -DLIBUSB_DIR="${EXTERNAL_LIBRARY_DIR}/libusb" -DCMAKE_C_FLAGS="-fPIC -O3"
make && make install
cd ..
rm -fr Build
cd ..
