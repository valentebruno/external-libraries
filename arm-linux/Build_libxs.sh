...
cd libxs
./autogen.sh
./configure --host=arm-linux --prefix="${EXTERNAL_LIBRARY_DIR}/libxs-${XS_VERSION}" --enable-static --disable-shared CC=aarch64-linux-gnu-gcc CXX=aarch64-linux-gnu-g++ CXXFLAGS="-fPIC -O2 -D_THREAD_SAFE"
make && make install
cd ..
